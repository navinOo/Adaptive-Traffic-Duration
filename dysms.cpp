...
int main() {
    ...
    while (cap.read(frame)) {
        ...
        for (int i = 0; i < LANE_COUNT; i++) {
            ...
            Scalar color = (i == currentGreen) ?
                           Scalar(0,255,0) : Scalar(0,0,255);
            Rect drawRect(0,
                          road_y + i * laneHeight,
                          frame.cols,
                          laneHeight);
            rectangle(frame, drawRect, color, 2);
            putText(frame,
                    "Density: " + to_string(lanes[i].smoothedDensity),
                    Point(20, road_y + i * laneHeight + 30),
                    FONT_HERSHEY_SIMPLEX,
                    0.7,
                    color,
                    2);
        }
        imshow("Traffic System", frame);
